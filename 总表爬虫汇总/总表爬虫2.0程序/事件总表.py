...
df = pd.read_excel(r'./investing彭博字段表_最终版(新版).xlsx', sheet_name='EP_EVENT(最新)')
df['Investing_Id'] = base_url + df['Investing_Id']
urllst = df['Investing_Id'].tolist()
...
for path_ in path_list:
    ...
    try:
        ...
        print(f'{name}未加载到数据库！！请注意检查！！') #未添加成功的原因很可能是因为sql中这个表的数据全为空，因此需要手动删掉这个表再添加