...
titlelst = ['US_EROPC_WoW','US_EWIC_WoW']
urllst = [base_url+df[df['TABLE_NAME']==i]['INVESTING_INDEX'].values[0] for i in titlelst]


# 储存txt文件
with ThreadPoolExecutor(max_workers=2) as t:  # 创建一个最大容纳数量为3的线程池
...
# csv存入数据库(最好在上传到数据库之前再做一次处理：去重、按照时间降序排列、时间格式的处理等等)
path_2 = rf'总表数据\处理好后的csv'
path_list = [os.path.join(path_2,i+'.csv') for i in titlelst]
for path_ in path_list:
